...
def loss_computation(y_true,y_hat,loss,batch_size,lambda_val,parameters,total_layers):
    '''
    Computes Cross-Entropy Loss/MSE loss
    ...
    total_layers: int
        total number of layers in Neural Network
...
class NeuralNetwork():
    ...
    def weight_bias_initialize(self,neurons_per_layer,init):
        ...
        Parameters:
            ...
            A list specifying number of neurons per layer including input and output layer
        ...
        Returns:
            ...
            old_parameters: dict
            momentum (u_t), used in momentum & nesterov gradient descent

            v,m: dict
            copy of old_parameters, used in adam, nadam & rmsprop
        '''
        ...
        for layer in range(1,len(self.neurons_per_layer)):
            ...
            elif self.init=='random': # Random normal
    ...
    def forward_propagation(self,data,parameters):
        ...
        Returns:
            ...
            activation: dict
                activation(H_i) at different layers
            
            pre_activation: dict
                pre_activation(A_i) at different layers
        ...
        for layer in range(1, self.num_layers):   # start from hidden layer 1
            self.Weight = self.parameters["W_"+str(layer)]
            self.bias = self.parameters["b_"+str(layer)]
            
            # a_i=W*h_(i-1) + b_i
            self.pre_activation["A_"+str(layer)] = np.matmul(self.Weight,self.activation["H_"+str(layer-1)]) + self.bias    
        
            if layer == self.num_layers-1:
                self.activation["H_"+str(layer)] = softmax(self.pre_activation["A_"+str(layer)])  # Final Output

            else:
                # h_i=g(a_i), g is the activation function
                if self.activation_function == 'sigmoid':
                    self.activation["H_"+str(layer)] = sigmoid(self.pre_activation["A_"+str(layer)]) 
        ...
        self.y_pred = self.activation["H_"+str(self.num_layers-1)]  #Final output
    ...
    def backpropagate(self,y_hat,y_true,activation,pre_activation,parameters):
        '''
        Backpropagation to calculate gradient of Loss wrt to weight and bias
        ...
        Parameters:
            ...
            pre_activation(a_i): dict

            parameters: dict
                Weight and bias at different layers
        
        Returns:
            parameter_gradient: dict
                gradient of loss wrt to weight and bias
  ...
  # Last_layer
        ...
        ## Reminder: derivative_softmax function works only at the final layer
  
        for layer in range(self.layers_no_input,0,-1):  # move from Hidden layer L-1 to Hidden layer 1
            self.parameter_gradient['dW_'+str(layer)]=(np.dot(self.gradient_dA['dA_'+str(layer)],self.activation["H_"+str(layer-1)].T)+
                                                       self.lambda_val*self.parameters['W_'+str(layer)])/self.batch_size
            
            self.parameter_gradient['db_'+str(layer)]=np.sum(self.gradient_dA['dA_'+str(layer)],axis=1,keepdims=True)/self.batch_size  
    
### For batch_size reference: https://datascience.stackexchange.com/questions/20139/gradients-for-bias-terms-in-backpropagation
    ...
    def predict(self,data, parameters):
        '''
        Predict final class labels (Not the encoded ones)
        ...
        Returns:
            predictions: final class labels
        '''
        self.data=data
        self.parameters=parameters
    ...
    def accuracy(self,true_labels,pred_labels):
        '''
        Find accuracy of the model (Train/Test/Val)
        '''
...
class NN_optimizers:
    '''
    Class for different gradient descent variants
    '''
    ...
    def __init__(self,parameters,gradients,learning_rate,old_parameters,v,m,t,num_layers):
        ...
        Parameters:
            ...
            old_parameters: dict
                used in nesterov, momentum gradient descent
            
            v: dict
                used in rmsprop, adam & nadam
            
            m: dict
                used in adam, nadam
            
            t: int
                timestep in adam, nadam
    ...
    def nesterov_gd(self,momentum): # Rewritten NAG
        '''
        ...
        for layer in range(1,self.num_layers):
            self.old_parameters['W_'+str(layer)]=self.momentum*self.old_parameters['W_'+str(layer)]+self.gradients['dW_'+str(layer)]
            self.parameters['W_'+str(layer)]=self.parameters['W_'+str(layer)]-self.learning_rate*(self.momentum*self.old_parameters['W_'+str(layer)]+self.gradients['dW_'+str(layer)])

            self.old_parameters['b_'+str(layer)]=self.momentum*self.old_parameters['b_'+str(layer)]+self.gradients['db_'+str(layer)]
            self.parameters['b_'+str(layer)]=self.parameters['b_'+str(layer)]-self.learning_rate*(self.momentum*self.old_parameters['b_'+str(layer)]+self.gradients['db_'+str(layer)])
    ...
    def adam(self,beta1,beta2,epsilon):
        ...
        for layer in range(1, self.num_layers):
            ...
            self.parameters["b_"+str(layer)] -= (self.learning_rate * mb_hat)/(np.sqrt(vb_hat + self.epsilon))
...
def early_stopping(val_loss,best_loss,best_epoch,patience,parameters,count):
    ...
    Parameters:
        ...
        best_loss: float
            Best Validation loss seen till current epoch
        best_epoch: int
    ...
    Returns:
        ...
        best_parameters: Best values of weight & bias
    '''
...
def main(args):
    ...
    def NN_fit_modified(train_data,train_labels,test_data,test_labels,test=False,patience=5):    # Pateince for early stopping  
        ...
        # initialize the parameters and old updates 
        parameters, old_parameters, v, m = NN.weight_bias_initialize(neurons_layer_wise,init=init) 
